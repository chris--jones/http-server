      ...
      fs.stat(file, (err, stat) => {
        if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
          if (req.statusCode === 404) {
            ...
            // Try to serve configured not found page e.g. ./404.html
            middleware({
              url: (handleError ? `/${path.join(baseDir, notFound)}` : req.url),
              headers: req.headers,
              statusCode: 404,
            }, res, next);
          }
          ...
          if (index !== 'false') {
            middleware({
              ...
              if (indexError) {
                status[500](res, next, { error: indexError });
                return;
              }
              if (opts.showDir) {
